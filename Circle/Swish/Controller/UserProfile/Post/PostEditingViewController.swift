...
//  Circle
//
//  Created by Jaehyeok Lim on 2/12/24.
//

import UIKit

class PostEditingViewController: BasicPostViewController {
    override func navigationBarLayout() {
        ...
        postEditingBarButton.snp.makeConstraints { make in
            make.width.equalTo(postEditingBarButton.titleLabel!.snp.width).offset(52)
        ...
        let righthStackview = UIStackView.init(arrangedSubviews: [userProfileBarButton, circularProgressBar, postEditingBarButton])
    ...
    override func addTagetsChild() {
        postEditingBarButton.addTarget(self, action: #selector(postingBarButtonAction), for: .touchUpInside)
    }
        ...
        if let userID = SharedProfileModel.myProfile.userID, let content = postTextView.text {
            if let postIndexPath = postIndexPathForEditing {
                if let postID = SharedPostModel.myPosts[postIndexPath].postID {
                    ...
                    updatePostData(postID: postID, updatedPostData: postData, updatedImages: selectedImages, userID: userID) { result in
